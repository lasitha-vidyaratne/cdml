...
def process_support_set_2(model: nn.Module, 
                        weights: List[torch.Tensor], 
                        X_train: torch.Tensor, 
                        y_train: torch.Tensor, 
                        num_classes: int) -> torch.Tensor:
    ...
    """
    # Compute input embeddings
    # support_embeddings1 = model.forward_weights(X_train, weights, 
    #     embedding=True)
    # print(support_embeddings1.shape)
    # print(X_train.shape)
    # exit()
    support_embeddings = X_train
    # Compute prototypes
    prototypes = torch.zeros((num_classes, support_embeddings.size(1)), 
        device=weights[0].device)
    for i in range(num_classes):
...
def process_query_set_2(model: nn.Module, 
                      weights: List[torch.Tensor], 
                      X_test: torch.Tensor, 
                      prototypes: torch.Tensor) -> torch.Tensor:
    """ Process the query set following the Matching Networks strategy.
